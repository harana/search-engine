...
use crate::{utils::replace_tilde_with_home_dir, FileCrawler};

/// Builder for a [`FileCrawler`] instance, allowing for more complex searches.
pub struct FileCrawlerBuilder {
...
impl FileCrawlerBuilder {
    /// Build a new [`Search`] instance.
    #[allow(deprecated)]
    pub fn build(&self) -> FileCrawler {
        FileCrawler::new(
    ...
    /// ### Examples
    /// ```rust
    /// use file_crawler::FileCrawlerBuilder;
    ///
    /// let search: Vec<String> = FileCrawlerBuilder::default()
    ///     .location("src")
    ///     .build()
    ///     .collect();
    /// ```
    ...
    /// ### Examples
    /// ```rust
    /// use file_crawler::{FileSize, FilterExt, FileCrawlerBuilder};
...
impl Default for FileCrawlerBuilder {
