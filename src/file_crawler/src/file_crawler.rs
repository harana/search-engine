...
use crate::{filter::FilterType, utils, FileCrawlerBuilder};
...
///
/// ```
/// use file_crawler::FileCrawler;
///
/// let search = FileCrawler::new("src", None, Some(".rs"), Some(1));
///
/// for path in search {
///    println!("{:?}", path);
/// }
...
pub struct FileCrawler {
    rx: Box<dyn Iterator<Item = PathBuf>>,
}
...
impl FileCrawler {
    ...
    pub(crate) fn new(
        ...
        walker.build_parallel().run(|| {
            let tx: Sender<PathBuf> = tx.clone();
...
impl Default for FileCrawler {
    ...
    fn default() -> Self {
        FileCrawlerBuilder::default().build()
    }
}
