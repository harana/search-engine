...
use tantivy::directory::{
    error::{
        DeleteError, LockError, OpenDirectoryError, OpenReadError,
        OpenWriteError,
    },
    AntiCallToken, Directory, DirectoryLock, Lock, TerminatingWrite, WatchCallback,
...
const KEYFILE: &str = "encryption.key";
// 16 byte random salt.
...
impl EncryptedMmapDirectory {
    fn new(store_key: KeyBuffer, path: &Path) -> Result<Self, OpenDirectoryError> {
        ...
        let (encryption_key, mac_key) = EncryptedMmapDirectory::expand_store_key(&store_key).map_err(|e|
            OpenDirectoryError::wrap_io_error(e, path.to_path_buf())
        )?;
    ...
    pub fn open_or_create<P: AsRef<Path>>(
        ...
        if passphrase.is_empty() {
            return Err({
                let error = IoError::new(ErrorKind::Other, "empty passphrase").into();
                OpenDirectoryError::wrap_io_error(error, path.as_ref().to_path_buf())
            });
        }

        if key_derivation_count == 0 {
            return Err({
                let error = IoError::new(ErrorKind::Other, "invalid key derivation count").into();
                OpenDirectoryError::wrap_io_error(error, path.as_ref().to_path_buf())
            });
        }
        ...
        let store_key = match key_file {
            ...
            Err(e) => {
                if e.kind() != ErrorKind::NotFound {
                    return Err(OpenDirectoryError::wrap_io_error(e, path.as_ref().to_path_buf()))
                }
    ...
    pub fn open<P: AsRef<Path>>(path: P, passphrase: &str) -> Result<Self, OpenDirectoryError> {
        ...
        let key_path = path.as_ref().join(KEYFILE);
        let key_file = File::open(key_path).map_err(|e| OpenDirectoryError::wrap_io_error(e, path.as_ref().to_path_buf()))?;
    ...
    fn load_store_key(
        ...
        key_file.read_exact(&mut version).map_err(|e| OpenDirectoryError::wrap_io_error(e, PathBuf::new()))?;
        key_file.read_exact(&mut iv).map_err(|e| OpenDirectoryError::wrap_io_error(e, PathBuf::new()))?;
        key_file.read_exact(&mut salt).map_err(|e| OpenDirectoryError::wrap_io_error(e, PathBuf::new()))?;
        let pbkdf_count = key_file.read_u32::<BigEndian>().map_err(|e| OpenDirectoryError::wrap_io_error(e, PathBuf::new()))?;
        key_file.read_exact(&mut expected_mac).map_err(|e| OpenDirectoryError::wrap_io_error(e, PathBuf::new()))?;
        ...
        if version[0] != VERSION {
            return Err({
                let error = IoError::new(ErrorKind::Other, "invalid index store version").into();
        ...
        let mut decryptor = Aes256Ctr::new_from_slices(&key, &iv).map_err(|e| {
            let error = IoError::new(ErrorKind::Other, format!("error initializing cipher {:?}", e));
            OpenDirectoryError::wrap_io_error(error, PathBuf::new())
        })?;

        let mut out = Zeroizing::new(encrypted_key);
        decryptor.try_apply_keystream(&mut out).map_err(|_| {
            let error = IoError::new(ErrorKind::Other, "Decryption error, reached end of the keystream.", );
            OpenDirectoryError::wrap_io_error(error, PathBuf::new())
        })?;
    ...
    fn encrypt_store_key(
        ...
        let mut encryptor = Aes256Ctr::new_from_slices(key, &iv).map_err(|e| {
            let error = IoError::new(ErrorKind::Other, format!("error initializing cipher: {:?}", e));
            OpenDirectoryError::wrap_io_error(error, key_path.to_path_buf())
        })?;

        let mut encrypted_key = [0u8; KEY_SIZE];
        encrypted_key.copy_from_slice(store_key);

        let mut key_file = File::create(key_path).map_err(|e| {
            OpenDirectoryError::wrap_io_error(e, key_path.to_path_buf())
        })?;

        // Write down our public salt and iv first, those will be needed to
        // decrypt the key again.
        key_file.write_all(&[VERSION]).map_err(|e| OpenDirectoryError::wrap_io_error(e, PathBuf::new()))?;
        key_file.write_all(&iv).map_err(|e| OpenDirectoryError::wrap_io_error(e, PathBuf::new()))?;
        key_file.write_all(salt).map_err(|e| OpenDirectoryError::wrap_io_error(e, PathBuf::new()))?;
        key_file.write_u32::<BigEndian>(pbkdf_count).map_err(|e| OpenDirectoryError::wrap_io_error(e, PathBuf::new()))?;

        // Encrypt our key.
        encryptor
            .try_apply_keystream(&mut encrypted_key)
            .map_err(|e| {
                let error = IoError::new(ErrorKind::Other, format!("unable to encrypt store key: {:?}", e));
                OpenDirectoryError::wrap_io_error(error, key_path.to_path_buf())
            })?;
        ...
        // the key.
        let mac =
            EncryptedMmapDirectory::calculate_hmac(VERSION, &iv, salt, &encrypted_key, hmac_key).map_err(|e| {
                OpenDirectoryError::wrap_io_error(e, key_path.to_path_buf())
            })?;
        let mac = mac.finalize();
        let mac = mac.into_bytes();
        key_file.write_all(mac.as_slice()).map_err(|e| OpenDirectoryError::wrap_io_error(e, key_path.to_path_buf()))?;

        // Write down the encrypted key.
        key_file.write_all(&encrypted_key).map_err(|e| OpenDirectoryError::wrap_io_error(e, key_path.to_path_buf()))?;
    ...
    fn generate_iv() -> Result<[u8; IV_SIZE], OpenDirectoryError> {
        ...
        rng.try_fill(&mut iv[..])
            .map_err(|e| {
                let error = IoError::new(ErrorKind::Other, format!("error generating iv: {:?}", e));
                OpenDirectoryError::wrap_io_error(error, PathBuf::new())
            })?;
        Ok(iv)
    }
    ...
    fn generate_key() -> Result<KeyBuffer, OpenDirectoryError> {
        ...
        rng.try_fill(&mut key[..]).map_err(|e| {
            let error = IoError::new(ErrorKind::Other, format!("error generating key: {:?}", e));
            OpenDirectoryError::wrap_io_error(error, PathBuf::new())
        })?;
        Ok(key)
    }
    ...
    fn derive_key(
        ...
        rng.try_fill(&mut salt[..]).map_err(|e| {
            let error = IoError::new(ErrorKind::Other, format!("error generating salt: {:?}", e));
            OpenDirectoryError::wrap_io_error(error, PathBuf::new())
        })?;
...
impl Directory for EncryptedMmapDirectory {
    fn open_read(&self, path: &Path) -> Result<FileSlice, OpenReadError> {
        ...
        let mut reader = AesReader::<Aes256Ctr, _>::new::<Hmac<Sha256>>(
            Cursor::new(source.read_bytes().map_err(|e| OpenReadError::wrap_io_error(e, path.to_path_buf()))?),
            &self.encryption_key,
            &self.mac_key,
            IV_SIZE,
            MAC_LENGTH,
    ...
    fn open_write(&self, path: &Path) -> Result<WritePtr, OpenWriteError> {
        let file = match self.mmap_dir.open_write(path)?.into_inner() {
            Ok(f) => f,
            Err(e) => {
                return Err(OpenWriteError::wrap_io_error(IoError::from(e), path.to_path_buf()));
            }
        };
