...
fn init_default_words() -> Result<()> {
    ...
    let buffer: &[u8] = include_bytes!("../stop_words");
    if !buffer.is_empty() {
...
pub struct PersistentStopWordManager {
    conn: StorageBackend,
...
impl PersistentStopWordManager {
    ...
    // TODO add-back #[instrument(name = "stop-words", skip_all)]
    pub fn new(conn: StorageBackend, manager: StopWordManager) -> Result<Self> {
