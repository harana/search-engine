...
use harana_common::tokio::sync::oneshot;
...
use crate::schema::{SchemaContext, ID_FIELD};
use crate::stop_words::{PersistentStopWordManager, StopWordManager};
use crate::storage::StorageBackend;
use crate::structures::{DocumentPayload, IndexContext};
...
impl WriterContext {
    ...
    // TODO add-back #[instrument(name = "buffer-calculator")]
    fn calculate_with_safe_buffer(&self) -> Result<WriterContext> {
        let mut sys = harana_common::sysinfo::System::new();
        sys.refresh_memory();
...
pub enum WriterOp {
    ...
    AddDocument(DocumentId, DocumentPayload),

    /// Adds multiple documents to the index.
    AddManyDocuments(Vec<(DocumentId, DocumentPayload)>),
...
impl IndexWriterWorker {
    ...
    // TODO add-back #[instrument(name = "index-writer", skip_all, fields(index = %self.index_name))]
    fn start(mut self) {
        ...
        loop {
            ...
            if (self.auto_commit == 0) | !op_since_last_commit {
                debug!("parking writer until new events present");
            ...
            match self.rx.recv_timeout(Duration::from_secs(self.auto_commit)) {
                ...
                Err(RecvTimeoutError::Disconnected) => {
                    debug!("writer actor channel dropped, shutting down...");
                    break;
                },
                Ok((op, waker)) => {
                    self.handle_message(op, waker);
    ...
    // TODO add-back #[instrument(name = "writer-message-handler", level = "trace", skip(self, waker))]
    fn handle_message(
        &mut self,
        op: WriterOp,
        waker: Option<oneshot::Sender<Result<()>>>,
    ) {
        debug!("ready to handling operations!");
    ...
    // TODO add-back #[instrument(name = "writer-op-handler", level = "trace", skip_all)]
    fn handle_op(&mut self, op: WriterOp) -> Result<()> {
        let (transaction_id, type_) = match op {
            ...
            WriterOp::AddDocument(document_id, document) => (self.handle_add_document(document_id, document)?, "ADD-DOCUMENT"),
            WriterOp::AddManyDocuments(documents) => {
                for document in documents {
                    let transaction_id = self.handle_add_document(document.0, document.1)?;
                    debug!(
    ...
    // TODO add-back #[instrument(name = "fast-fuzzy-frequencies", level = "info", skip_all)]
...
// TODO add-back #[instrument(name = "writer-actor", level = "trace", skip_all, fields(name = %name, auto_commit = auto_commit, fast_fuzzy = using_fast_fuzzy))]
fn start_writer(
    name: String,
    conn: StorageBackend,
    reader: Reader,
    ...
    let pk_field = schema.get_field(ID_FIELD).unwrap();
...
impl Writer {
    ...
    // TODO add-back #[instrument(name = "index-writer", skip_all, fields(index = %ctx.name))]
    pub fn create(
        ctx: &IndexContext,
        reader: Reader,
    ) -> Result<Self> {
    ...
    // TODO add-back #[instrument(name = "writer-message-emitter", skip(self), fields(index = %self.index_name))]
    pub async fn send_op(&self, op: WriterOp) -> Result<()> {
        ...
        loop {
            ...
            info!("Tantivy operation queue full, waiting for wakeup");
    ...
    // TODO add-back #[instrument(name = "writer-shutdown", skip(self), fields(index = %self.index_name))]
