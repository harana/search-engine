...
pub struct ThumbnailerGif;

#[async_trait]
impl Thumbnailer for ThumbnailerGif {
    ...
    async fn thumbnail(&self, source_file: &Path, target_file: &Path, _document_id: u64, _app: &'static AppHandle<Wry>, width: u32, height: u32) -> Result<()> {
        ImageDecoder::standard(source_file, target_file, ImageType::Gif, true, None, (width, height)).await
    }
...
mod tests {
    ...
    use harana_thumbnailer_core::thumbnailer::Thumbnailer;
    use crate::thumbnailer_gif::ThumbnailerGif;
