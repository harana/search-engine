...
pub struct ThumbnailerJpeg;

#[async_trait]
impl Thumbnailer for ThumbnailerJpeg {
    ...
    async fn thumbnail(&self, source_file: &Path, target_file: &Path, _document_id: u64, _app: &'static AppHandle<Wry>, width: u32, height: u32) -> Result<()> {
        ImageDecoder::standard(source_file, target_file, ImageType::Jpeg, false, None, (width, height)).await
    }
...
mod tests {
    ...
    use crate::thumbnailer_jpeg::ThumbnailerJpeg;

    #[tokio::test]
    async fn test_indexing() {
        let now = Instant::now();

        let _ = ThumbnailerJpeg.thumbnail(
            Path::new("../../../test_files/Sample1.jpg"),
