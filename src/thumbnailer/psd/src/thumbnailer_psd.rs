...
pub struct ThumbnailerPsd;

#[async_trait]
impl Thumbnailer for ThumbnailerPsd {
    ...
    async fn thumbnail(&self, source_file: &Path, target_file: &Path, _document_id: u64, _app: &'static AppHandle<Wry>, width: u32, height: u32) -> Result<()> {
        ImageDecoder::standard(source_file, target_file, ImageType::Psd, true, None, (width, height)).await
    }
...
mod tests {
    ...
    use crate::thumbnailer_psd::ThumbnailerPsd;

    #[tokio::test]
    async fn test_indexing() {
        let now = Instant::now();

        let _ = ThumbnailerPsd.thumbnail(
            Path::new("../../../test_files/Sample1.psd"),
