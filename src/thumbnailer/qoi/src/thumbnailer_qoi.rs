...
pub struct ThumbnailerQoi;

#[async_trait]
impl Thumbnailer for ThumbnailerQoi {
    ...
    async fn thumbnail(&self, source_file: &Path, target_file: &Path, _document_id: u64, _app: &'static AppHandle<Wry>, width: u32, height: u32) -> Result<()> {
        ImageDecoder::standard(source_file, target_file, ImageType::Qoi, false, None, (width, height)).await
    }
...
mod tests {
    ...
    use harana_thumbnailer_core::thumbnailer::Thumbnailer;
    use crate::thumbnailer_qoi::ThumbnailerQoi;

    #[tokio::test]
    async fn test_indexing() {
        let now = Instant::now();

        let _ = ThumbnailerQoi.thumbnail(
            Path::new("../../../test_files/Sample1.qoi"),
