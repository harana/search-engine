...
use harana_common::rand::{distributions::Alphanumeric, Rng, thread_rng};
use image::{GenericImage, ImageBuffer, Rgba, RgbaImage};
use imageproc::{drawing::draw_text_mut, rect::Rect};
use rayon::prelude::*;
use rusttype::{Font, point, Point, PositionedGlyph, Scale};
use textwrap::wrap;
...
pub fn select_sharpest_images(media_attributes: &MediaAttributes, media_capture: &MediaCapture, args: &Args) -> Result<(Vec<Frame>, Vec<Frame>), VcsrError> {
    ...
    let do_capture = |ts_tuple: (f32, String),
        ...
        let file_name = format!("tmp{}{}", rand_string, suffix);
        dir.push(&file_name);
...
pub fn prepare_metadata_text_lines(
    ...
    let template = format!(
        ...
        file_name = media_attributes.file_name,
...
pub fn compose_contact_sheet(
    ...
    for (i, frame) in frames.iter().enumerate() {
        let mut f = image::open(&Path::new(&frame.file_name)).unwrap().to_rgba8();
