...
pub struct ThumbnailerWebp;

#[async_trait]
impl Thumbnailer for ThumbnailerWebp {
    ...
    async fn thumbnail(&self, source_file: &Path, target_file: &Path, _document_id: u64, _app: &'static AppHandle<Wry>, width: u32, height: u32) -> Result<()> {
        ImageDecoder::standard(source_file, target_file, ImageType::Webp, false, None, (width, height)).await
    }
...
mod tests {
    ...
    use crate::thumbnailer_webp::ThumbnailerWebp;
